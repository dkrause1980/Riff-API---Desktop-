...
if (isset($_POST['codigo2'])) {
    if ($_POST['activo2']=="0") {
        ...
        $cargado = true;
        $json_data = cargar_cod_rep($_POST['codigo2']);
    }
}
...
<head>
    <title>C&oacute;digos de Reparación</title>
...
<body>
    ...
    <nav>
        <div class="nav-wrapper blue lighten-4" >
            <div class="col s12">
                ...
                <a href="#!" class="breadcrumb black-text">C&Oacute;DIGOS DE REPARACI&Oacute;N</a>
            </div>
        </div>
    </nav>
    ...
    <div class="contenido">
        <div class="row">
                ...
                <div class="row">
                    <div class="input-field col s1 tooltipped" data-position="bottom" data-tooltip="Sólo 3 números">
                        <input id="cod_rep" type="number" required class="validate" value="<?php if ($cargado == true) {
                                                                                            echo $json_data['codigo'];
                                                                                        } ?>" <?php if ($cargado == true) {
                                                                                                    echo "disabled";
                                                                                                } ?>>
                        <label for="cod_rep">Cód de reparación</label>
                    </div>
                    ...
                    <div class="input-field col s1">
                        <select id="activo" required name="activo">
                ...
                <div class="row">
                    <div class="col s1" style="margin-top: 20px;">
                        <button id="bt_agregar_cod_rep" <?php if ($cargado == true) {
                                                    echo "disabled";
                                                } ?> onclick="agregar_cod_rep()" class="waves-effect waves-light btn-small">
                            AGREGAR
                        </button>
                    </div>
                    <div class="col s1" style="margin-top: 20px;">
                        <button id="bt_modificar_cod_rep" <?php if ($cargado != true) {
                                                        echo "disabled";
                                                    } ?> onclick="modificar_cod_rep()" class="waves-effect waves-light btn-small">
                            GUARDAR
                        </button>
                    </div>
                    <div class="col s1" style="margin-top: 20px;">
                        <button id="nuevo_cod_rep" onclick="blanquear_cod_rep()" class="waves-effect waves-light btn-small">
                            NUEVO
                        </button>
                    </div>
                </div>
        ...
        <table class="highlight centered" id="tabla_cod_rep">
            <?php $json = get_cod_rep();
            ?>
            ...
            <?php
                ...
                <tr>
                    <td><?=$json[$i]['cod_resolucion'];?></td>
                    <td><?=$json[$i]['desc_codigo'];?></td>
                    <td><?=$json[$i]['activo'];?></td>
    ...
    <form id="cargar_rep" method="POST" action="abm_cod_rep.php">
    ...
    <form id="modif_rep" method="POST" action="abm_cod_rep.php">
        <input id="codigo2" type="hidden" name="codigo2">
        <input id="activo2" type="hidden" name="activo2">
    </form>

    <form id="cargar_modif_rep" method="POST" action="abm_cod_rep.php">
    ...
    TableSorter.makeSortable(document.getElementById("tabla_cod_rep"));
    };
  </script>
